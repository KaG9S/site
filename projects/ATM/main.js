...
function getMoney(){
    ...
    if(money % 1 == 0 && money >= 10){
        if(money % 10 == 0){
            ...
            result.innerText = 'Операція призупинена! Наразі у банкоматі немає купюр, які менше 10₴. Ми можемо віддати вам '+(money - (money % 10))+'₴ або повернути гроші.'
            if(prompt('Напишіть число '+rand+' якщо хочете повернути кошти. В інакшому разі кошти будуть списані')){
                result.innerText = 'Операція успішна! Ось ваші '+(money - (money % 10))+'₴'
                for(let i = 0; i <= (money - (money % 10)); i++){
                    resultDiv.innerHTML += '<img src="гривня.jpg" alt="1 гривня" style="border: 3px black solid;">'
                }
            }else{
